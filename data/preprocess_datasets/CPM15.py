...
def process_cpm17_dataset(input_base_dir, output_base_dir):
    """
    Process the CPM-17 dataset and convert instance masks to semantic masks (background/nucleus).

    Args:
        input_base_dir (str): Root directory of the input dataset (e.g., /path/to/.../cpm17/train/).
        output_base_dir (str): Root directory of the output dataset.
    """
    # Define input subdirectories
    images_input_dir = os.path.join(input_base_dir, "Images")
    labels_input_dir = os.path.join(input_base_dir, "Labels")
    ...
    if not os.path.exists(labels_input_dir):
        print(f"Error: Labels directory {labels_input_dir} does not exist.")
        return
    ...
    print(f"Processing CPM-17 dataset from: {input_base_dir}")
    print(f"Output will be saved to: {output_base_dir}")
    print("Converting instance masks to semantic masks (0: background, 1: nucleus).")
    ...
    if len(img_files) != len(mat_files):
        print(f"Warning: Number of images ({len(img_files)}) and .mat labels ({len(mat_files)}) do not match. Using minimum count.")
    ...
    for i in range(min_count):
        ...
        if img_name_base != mat_name_base:
            print(f"Warning: Image name '{img_name_base}' does not match .mat name '{mat_name_base}'. Processing based on list order.")
        ...
        unique_base_name = f"cpm15_semantic_sample_{processed_count:06d}"
...
if __name__ == "__main__":
    ...
    INPUT_BASE_DIR = "/path/to/PFM_Segmentation_Data/cpm15"
    OUTPUT_BASE_DIR = "/path/to/PFM_Segmentation_Data/cpm15/processed_semantic_dataset/" # Modify this path as needed

    print("Starting CPM-17 dataset processing for semantic segmentation...")
    process_cpm17_dataset(INPUT_BASE_DIR, OUTPUT_BASE_DIR)
    print("CPM-17 dataset processing for semantic segmentation completed.")