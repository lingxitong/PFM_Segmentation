...
def process_kumar_dataset_unified(input_base_dir, output_base_dir):
    """
    Process Kumar train/test_same/test_diff splits, convert instance masks to semantic masks (background/nucleus), and write unified outputs.

    Args:
        input_base_dir (str): Root directory of the input dataset (e.g., /path/to/.../kumar/).
        output_base_dir (str): Root directory of the output dataset.
    """
    # Define subdirectories to process
    subdirs_to_process = ['train', 'test_same', 'test_diff'] # Specify the subdirectories to process
    ...
    print(f"Processing Kumar dataset from subdirectories: {subdirs_to_process}")
    ...
    for subdir_name in subdirs_to_process:
        ...
        # Original images are .tif
        img_files = sorted([f for f in os.listdir(images_input_dir) if f.lower().endswith('.tif')])
        ...
        for i in range(min_count):
            ...
            unique_base_name = f"kumar_{subdir_name}_sample_{processed_count:06d}"
            ...
            # Copy and convert image (from .tif to .png)
            try:
...
if __name__ == "__main__":
    ...
    INPUT_BASE_DIR = "/path/to/PFM_Segmentation_Data/kumar/"
    OUTPUT_BASE_DIR = "/path/to/PFM_Segmentation_Data/kumar/" # Modify this path as needed

    print("Starting unified Kumar dataset processing for semantic segmentation (train, test_same, test_diff)...")
    process_kumar_dataset_unified(INPUT_BASE_DIR, OUTPUT_BASE_DIR)
    print("Unified Kumar dataset processing for semantic segmentation completed.")