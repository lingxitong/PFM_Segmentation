...
def visualize_prediction(image: Union[torch.Tensor, np.ndarray],
    ...
    # Comment out plt.show() to avoid blocking training (not needed when running in the background)
    # plt.show()
...
def create_interactive_training_dashboard(training_stats: Dict[str, List[float]],
    ...
    # Comment out fig.show() to avoid blocking training (not needed when running in the background)
    # fig.show()
